...
//                        Log.e("aa", "-----------获取用户信息==onSuccess=" + info[0]);
...
//                    }
//                });
        OkHttpUtils.get().url(new HttpClient().mUrl + "User.getBaseInfo")
                .addParams("uid", uid)
                .addParams("token", token)
                .build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
                Log.e("aa","---------User.getBaseInfo=onError="+e.getMessage());
                if (commonCallback != null) {
                    commonCallback.callback(null);
                }
            }
            ...
            public void onResponse(String response, int id) {
                try {
                    ...
                    if(code==0) {
                        ...
                        JSONObject obj = JSON.parseObject(info.get(0) + "");
                        UserBean bean = JSON.toJavaObject(obj, UserBean.class);
                        CommonAppConfig.getInstance().setUserBean(bean);
                        CommonAppConfig.getInstance().setUserItemList(obj.getString("list"));
                        SpUtil.getInstance().setStringValue(SpUtil.USER_INFO, info.get(0) + "");
                        if (commonCallback != null) {
                            commonCallback.callback(bean);
                        }
                    }else {
     ...
     * 充值页面 支付宝微信合体支付
     */
    public static void HomeGetPayUrl(HttpCallback callback) {
        HttpClient.getInstance().get("Home.GetPayUrl", MainHttpConsts.GET_BALANCE)
                .params("uid", CommonAppConfig.getInstance().getUid())
                .params("token", CommonAppConfig.getInstance().getToken())
                .execute(callback);
    }
