...
import com.yunbao.main.activity.HomVideoClassActivity;
...
import com.yunbao.main.bean.HomeVideoTItleBean;
...
public abstract class NewMainHomeVideoViewHolder extends AbsMainHomeChildViewHolder {
    ...
    public NewMainHomeVideoViewHolder(Context context, ViewGroup parentView, boolean shouBanner1) {
    ...
    public void initView() {
        ...
        LiveVideoReclyViewHolder reclyViewHolder1 = new LiveVideoReclyViewHolder(mContext, viewPager) {
            @Override
            public Observable<List<LiveVideoBean>> requestData(int p) {
                return getHot(p);
            }
        };
        LiveVideoReclyViewHolder reclyViewHolder2 = new LiveVideoReclyViewHolder(mContext, viewPager) {
            @Override
            public Observable<List<LiveVideoBean>> requestData(int p) {
                return getPopular(p);
            }
...
//        ConfigBean configBean = CommonAppConfig.getInstance().getConfig();
    ...
    public abstract Observable<List<LiveVideoBean>> getHot(int p);

    public abstract Observable<List<LiveVideoBean>> getPopular(int p);
...
//    public abstract Observable<List<LiveBean>> getNew(int p);
    ...
    public void SlideGetSlideList() {
        ...
        LiveHttpUtil.SlideGetSlideList("3", new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    ...
                    for (int i = 0; i < bannerBean.getList().size(); i++) {
                        if (bannerBean.getList().get(i).getSlide_cid().equals("3")) {
                            bannerList.add(CommonAppConfig.HOST +bannerBean.getList().get(i).getSlide_pic());
                        }
                    }
                    //设置图片加载器
    ...
    public void VideoGetVideoClassify() {
        list.clear();
        LiveHttpUtil.VideoGetVideoClassify(new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
                    Gson gson = new Gson();
                    HomeVideoTItleBean bannerBean = gson.fromJson(info[0], HomeVideoTItleBean.class);
                    for (int i = 0; i < bannerBean.getList().size(); i++) {
                        LiveClassBean bean = new LiveClassBean();
                        bean.setId(Integer.valueOf(bannerBean.getList().get(i).getId()));
                        bean.setName(bannerBean.getList().get(i).getName());
                        bean.setDes(bannerBean.getList().get(i).getDes());
                        bean.setThumb(bannerBean.getList().get(i).getThumb());
                        list.add(bean);
                    }
                        ...
                        topAdapter.setOnItemClickListener(new OnItemClickListener<LiveClassBean>() {
                            ...
                            public void onItemClick(LiveClassBean bean, int position) {
                                ...
                                if (bean.isAll()) {//全部分类
                                    ...
                                    HomVideoClassActivity.forward(mContext, bean.getId(), bean.getName());
                                }
                            }
                        });
